...
def simple_detect_ultrasound_anomalies():
    config_path = os.path.join(CFG_ROOT, "detect_ultrasound_anomalies.yaml")
    with open(config_path) as stream:
        try:
            ...
            for idx, img in enumerate(image_list):
                mask = aesculapius.detect_ultrasound_anomalies(img, config)

                original_name = f"{Path(folder_path).stem}_{idx}"
                output_name = f"predicted_{original_name}.png"
...
if __name__ == "__main__":
    simple_detect_ultrasound_anomalies()
