...
def brighten_all_ultrasound_images():
    config_path = os.path.join(
        CFG_ROOT, "update_ultrasound_brightness_manual.yaml")
    with open(config_path) as stream:
        try:
            ...
            for idx, img in enumerate(image_list):
                enhanced = aesculapius.update_ultrasound_brightness_manual(
                    img, config
                )

                original_name = f"{Path(folder_path).stem}_{idx}"
                output_name = f"enhanced_brightness_{original_name}.png"
                output_file = Path(
                    os.path.join(
                        out_path,
                        output_name))
...
if __name__ == "__main__":
    brighten_all_ultrasound_images()
